...
2. `due_diligence_engine/cli.py` の `@click.version_option(version=...)`
3. 両 README のバッジ `version-vX.Y.Z`
4. 両 README フッターの `` `vX.Y.Z` `` 行

## モデル ID / Model IDs

- Claude モデル ID は `due_diligence_engine/config.py` の `MODELS` 辞書で一元管理。
