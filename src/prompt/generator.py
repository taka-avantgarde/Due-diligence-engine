...
4. **Ultra-High Security (50% — THE CORE of Atlas philosophy)**
   ...
   - **4b. Privacy Protection (8%)** — GDPR/CCPA/HIPAA depth, data minimization, consent granularity

   - **4c. General Security Posture (2% — MINIMAL)** — "anyone can add MFA, SOC2, WebAuthn".
     Checkbox compliance is minimum hygiene, not differentiation.
...
### Weight Philosophy
Atlas believes **cryptographic sophistication differentiates winners** in privacy-conscious markets.
Companies with SOC2 but plaintext storage score LOW on axis 4.
Companies using libsignal + PQXDH + zero self-rolled crypto score HIGH.
...
"atlas_four_axis": {
  "axes": [
    ...
    {
      "axis_key": "security",
      "name_en": "Ultra-High Security", "name_ja": "超高度セキュリティ",
      "weight_pct": 50,
      "score": 0-100, "level": 1-10,
      "rationale": "...",
      "sub_items": [
...
4. **超高度セキュリティ Ultra-High Security (50% — Atlas 哲学の核心)**
   ...
   - **4b. プライバシー保護（8%）** — GDPR/CCPA/HIPAA 深度、データ最小化、同意管理の粒度

   - **4c. 一般セキュリティ態勢（2% — 最小扱い）** — 「MFA、SOC2、WebAuthn は誰でもできる」。
     チェックリスト準拠は最低限の衛生であり差別化要因ではない。
...
### 重み哲学
Atlas は **プライバシー重視市場では暗号化の高度さが勝者を分ける** と信じています。
SOC2 取得済みだが平文保存の企業は軸4で低評価。
libsignal + PQXDH + 自前crypto不在の企業は高評価。
...
**Posture (2, minimal weight)**:
- `mfa_webauthn` — multi-factor auth, WebAuthn
- `soc2_iso27001` — SOC2 Type II or ISO 27001 certification
...
**Posture（2、最小重み）**: `mfa_webauthn`, `soc2_iso27001`
...
```json
{
  ...
  "atlas_four_axis": {
    "axes": [
      ...
      {"axis_key": "security",     "name_en": "Ultra-High Security",  "name_ja": "超高度セキュリティ", "weight_pct": 50, "score": 0-100, "level": 1-10, "rationale": "...",
        "sub_items": [
