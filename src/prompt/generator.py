...
> **How to use**: This prompt was generated by [DDE (Due Diligence Engine)](https://github.com/Atlas-Associates-Inc/Due-diligence-engine).
...
> **使い方**: このプロンプトは [DDE (Due Diligence Engine)](https://github.com/Atlas-Associates-Inc/Due-diligence-engine) が自動生成しました。
...
def generate_consulting_prompt(
    ...
    # English base prompt for en + all other languages; the PDF command and an
    # explicit output-language directive localize the generated report.
    prompt = _generate_consulting_en(result, stage, urls=urls, pdf_lang=lang)
    if lang != "en":
        prompt += _output_language_directive(lang)
    return prompt


# Languages whose REPORT CONTENT is generated by the AI in that language.
...
def _output_language_directive(lang: str) -> str:
    """Append a directive instructing the AI to write all report CONTENT in `lang`."""
    name = _LANG_OUTPUT_NAMES.get(lang)
    if not name:
        return ""
    return (
        f"\n\n## ⚠️ OUTPUT LANGUAGE — {name}\n"
        f"Write EVERY human-readable VALUE in the JSON (executive/business summaries, "
        f"SWOT points and explanations, rationales, recommendations, risks, strengths, "
        f"future-outlook prose, strategic advice, investment thesis, competitor rationales, "
        f"site-verification notes, glossary text) in {name}. "
        f"Keep all JSON KEYS and enum values (verdict, severity, confidence, "
        f"status such as verified/claimed/not_implemented/unknown) in English. "
        f"Keep brand names, model IDs, and technical acronyms unchanged.\n"
    )


def _generate_consulting_en(result: AnalysisResult, stage: str, *, urls: list[str] | None = None, pdf_lang: str = "en") -> str:
    sections: list[str] = []

    sections.append(_CONSULTING_HEADER_EN)
    # STEP 0: force the AI to refresh market/competitor data via web search
    # BEFORE diving into code reading. This must come right after the header so
    # it is the very first task the AI sees.
    sections.append(_WEB_RESEARCH_MANDATE_EN)
    ...
    sections.append("## Scoring Framework (5 dimensions, 10-level scale)\n")
    for dim in DIMENSIONS_EN:
        sections.append(
            f"### {dim['name']} (Weight: {dim['weight']})\n"
            f"**Evaluate**: {dim['what']}\n"
    ...
    sections.append(_consulting_pdf_command(result, pdf_lang))
...
def _generate_consulting_ja(result: AnalysisResult, stage: str, *, urls: list[str] | None = None) -> str:
    ...
    # STEP 0: コード読取の前に Web 検索で市場・競合データを最新化させる。
    # ヘッダー直後に置くことで AI が最初に目にする指示にする。
    sections.append(_WEB_RESEARCH_MANDATE_JA)
    ...
    sections.append("## スコアリング基準（5次元 × 10段階評価）\n")
    for dim in DIMENSIONS_JA:
        sections.append(
            f"### {dim['name']}（重み: {dim['weight']}）\n"
            f"**評価観点**: {dim['what']}\n"
...
> ⚠️ **Reminder from STEP 0**: Before populating this section, you MUST have run the
> web searches mandated in the "Live Web Research" block at the top of this prompt.
> Every competitor name, position, and metric here must be cross-checked against
> 2025-2026 sources. Stale, training-cutoff-only data is not acceptable.
...
> ⚠️ **STEP 0 リマインダー**: このセクションを埋める前に、プロンプト冒頭の
> 「ライブ Web 調査」ブロックで指定された Web 検索を必ず実施済みであること。
> ここに記載する競合名・ポジション・各種数値は、すべて 2025〜2026 年の
> ソースとクロスチェックされている必要があります。学習データのみに依拠した
> 古い情報は許容しません。

対象企業について **固定6市場** にわたる包括的な競合分析を作成してください。
**7つのチャートタイプ** それぞれについて、全6市場のデータを生成します（1ページ = 2×3グリッド表示）。
...
_CONSULTING_HEADER_EN = """# Consulting-Grade Technical Due Diligence

## ⚠️ CRITICAL: Identifying the Analysis Target

**The target project to analyze is the project whose data appears in the
"Heuristic Analysis Results" section below — NOT DDE (Due Diligence Engine) itself.**

DDE is the tool that generated this prompt. It was run against another project.
Do NOT analyze DDE's source code, philosophy, or competitors.
Analyze the project described in the Heuristic Analysis section.

If the heuristic data shows e.g. a messaging app, analyze that messaging app.
If it shows e.g. a fintech API, analyze that fintech API.
**The heuristic data IS the target. Read its files, not DDE's.**
...
_CONSULTING_HEADER_JA = """# コンサルティンググレード テクニカルデューデリジェンス

## ⚠️ 最重要: 分析対象の特定

**分析対象は、後述の「ヒューリスティック分析結果」セクションに記述されている
プロジェクトです。DDE (Due Diligence Engine) ツール本体ではありません。**

DDE はこのプロンプトを生成したツールです。別のプロジェクトを対象に実行されました。
**DDE のソースコード・哲学・競合は分析しないでください。**
ヒューリスティック分析セクションに記述されたプロジェクトを分析してください。

ヒューリスティックデータがメッセージングアプリを示しているなら、そのメッセージングアプリを分析。
フィンテック API を示しているなら、そのフィンテック API を分析してください。
**ヒューリスティックデータが対象。そのファイルを読む（DDE のファイルではない）。**
...
# ---------------------------------------------------------------------------
# Web Research Mandate (v0.4) — force AI to refresh market/competitor data
# via web search BEFORE committing to analysis. Inserted at the very top of
# the consulting prompt so it takes precedence over downstream sections.
# ---------------------------------------------------------------------------

_WEB_RESEARCH_MANDATE_EN = """## 🌐 STEP 0 — Live Web Research (MANDATORY, execute BEFORE code reading)

> Your training data has a cutoff. **The DD report must reflect the latest
> market state, not what you learned during training.** Run web searches FIRST,
> then evaluate the code with that fresh context.

### Required searches (use WebSearch / WebFetch / browser tools)

**1. Competitor landscape** — critical for the 7 competitive charts and the implementation matrix
   - `<target industry> top competitors 2026`
   - For each candidate competitor: `<name> acquisition OR shutdown OR pivot 2025 2026`
   - Verify which competitors are still operating, which were acquired, which new entrants emerged
   - Examples: `E2EE messaging companies 2026`, `Signal alternative 2026`, `LLM observability vendors 2026`

**2. Funding & traction** — affects market_presence, revenue multiples, comparables
   - `<each major competitor> Series B OR Series C OR funding 2025 2026`
   - `<target industry> latest funding rounds 2026`
   - Refresh valuation / ARR / employee count estimates

**3. Technology trends** — affects technology_advancement scoring
   - `<core tech stack> latest version 2026` (e.g., `Rust async runtime 2026`, `PyTorch 2.6 features`)
   - `<key algorithm/protocol> state of the art 2026`
   - Do NOT penalize the target for not using bleeding-edge tech if mature alternatives are appropriate

**4. Security landscape** — affects red flags & security maturity
   - `<each major dependency> CVE 2025 2026`
   - `<target language/runtime> security advisories 2026`
   - Cross-reference OSV when possible: `https://osv.dev/list?ecosystem=PyPI&q=<package>`

**5. Regulatory / market shifts**
   - `<target jurisdiction> <industry> regulation 2026` (e.g., `EU AI Act enforcement 2026`)
   - `<target customer segment> market size 2026 forecast`

### Output: add a `data_freshness` block at the TOP of the JSON
...
"data_freshness": {
  ...
  "data_cutoff_warning": null
}
```

### If web search is unavailable in your runtime

Set `web_search_performed: false` and write a clear `data_cutoff_warning` such as
`"WebSearch tool unavailable in this terminal — analysis based on training-data baseline only (cutoff ~2025)."`
**Never silently produce stale data.** The reader must know what is fresh and what is not.

### Per-competitor citations (mandatory)

For every entry in `competitive_analysis.markets[].competitors[]` and
`implementation_matrix.competitors[]`, include a `sources` array with at least
**1 verifying URL** (their website, recent news, GitHub repo, funding press release, SEC filing, etc.).
For each `implementation_matrix` cell marked `verified` or `claimed`, the source URL must be the
specific page where the evidence was found.

---
"""

_WEB_RESEARCH_MANDATE_JA = """## 🌐 STEP 0 — ライブ Web 調査（必須、コード読取の前に実行）

> あなたの学習データにはカットオフがあります。**本 DD レポートは学習時の知識ではなく、
> 最新の市場状態を反映する必要があります。** まず Web 検索を実行し、その鮮度ある文脈で
> コードを評価してください。

### 必須の検索（WebSearch / WebFetch / ブラウザツールを使用）

**1. 競合ランドスケープ** — 7つの競合チャートと実装能力マトリックスに必須
   - `<対象業界> top competitors 2026`
   - 各候補競合について: `<企業名> acquisition OR shutdown OR pivot 2025 2026`
   - 存続中の競合、買収/合併された競合、新規参入を確認
   - 例: `E2EE messaging companies 2026`、`Signal alternative 2026`、`LLM observability vendors 2026`

**2. 資金調達・トラクション** — market_presence、収益マルチプル、類似企業比較に影響
   - `<主要競合各社> Series B OR Series C OR funding 2025 2026`
   - `<対象業界> latest funding rounds 2026`
   - バリュエーション / ARR / 従業員数の推定を更新

**3. 技術トレンド** — technology_advancement スコアに影響
   - `<コア技術スタック> latest version 2026`（例: `Rust async runtime 2026`、`PyTorch 2.6 features`）
   - `<主要アルゴリズム/プロトコル> state of the art 2026`
   - 成熟版で十分な場合、最先端を使っていないことで減点しない

**4. セキュリティ動向** — レッドフラグ・セキュリティ成熟度に影響
   - `<主要依存ライブラリ各種> CVE 2025 2026`
   - `<対象言語/ランタイム> security advisories 2026`
   - 可能なら OSV を参照: `https://osv.dev/list?ecosystem=PyPI&q=<package>`

**5. 規制・市場変化**
   - `<対象法域> <業界> regulation 2026`（例: `EU AI Act enforcement 2026`）
   - `<対象顧客セグメント> market size 2026 forecast`

### 出力: JSON 先頭に `data_freshness` ブロックを追加
...
`web_search_performed: false` とし、`data_cutoff_warning` に明示的な警告を記述すること。例:
`"このターミナルでは WebSearch ツールが利用不可。学習データ（カットオフ約 2025 年）のみに基づく分析。"`
**鮮度の無いデータを黙って出力しないこと。** 読み手が「何が最新で何が古いか」を判断できる必要があります。

### 競合ごとの出典（必須）

`competitive_analysis.markets[].competitors[]` と `implementation_matrix.competitors[]` の
全エントリに `sources` 配列を含めること。最低 **1つの検証 URL**（公式サイト・最近のニュース・
GitHub リポ・資金調達プレスリリース・有報など）。実装能力マトリックスの `verified` / `claimed`
セルでは、**証拠が見つかった具体的なページ URL** を記載すること。

---
"""


_ATLAS_FOUR_AXIS_INSTRUCTIONS_EN = """## Atlas Optimization Assessment (v2.0, additional to 5-dimension scoring)

⚠️ Reminder: the target is the project in the Heuristic Analysis section, NOT DDE itself.

Evaluate the target on **Atlas Associates' engineering philosophy** — a parallel 4-axis view
in addition to (not replacing) the existing 5-dimension scoring. Atlas Associates is the
**author of DDE** (this evaluation tool), not the target. Apply Atlas's philosophy to evaluate
THE TARGET PROJECT, regardless of what industry the target is in.

### 4 Axes (weights sum to 100%)

1. **Performance (20%)** — algorithmic efficiency × network optimization × caching strategy ×
...
4. **Security Strength (55% — THE CORE of Atlas philosophy)**

   Sub-breakdown (non-public weights, industry-adjusted, encryption-dominant):

   - **4a. Encryption Sophistication (35% of 55% = core)** — Signal Protocol, PQXDH, libsignal/BoringSSL
...
### Weight Philosophy
Atlas places strong emphasis on **cybersecurity defense overall** (encryption +
privacy + communications + layered defense) at 55%, with cryptographic
implementation depth (35%) as the largest sub-weight — **not because encryption
alone matters, but because all four sub-categories together represent serious
cybersecurity engineering**. Third-party certifications are **never scored**:
- A SOC2-certified plaintext-storage service still scores LOW on axis 4.
- An uncertified service with libsignal + PQXDH + sealed-sender + defense-in-depth still scores HIGH.
    ...
    },
    {
      ...
      "sub_items": [
        {"key": "encryption", "name_en": "Cryptographic Sophistication", "name_ja": "暗号化技術の高度さ",
         "score": 0-100, "level": 1-10, "weight_pct": 35, "rationale": "..."},
        {"key": "privacy", "name_en": "Privacy Protection", "name_ja": "プライバシー保護",
         "score": 0-100, "level": 1-10, "weight_pct": 8, "rationale": "..."},
        {"key": "posture", "name_en": "Basic Hygiene (MFA/WebAuthn)", "name_ja": "基本衛生（MFA・WebAuthn）",
...
_ATLAS_FOUR_AXIS_INSTRUCTIONS_JA = """## Atlas 最適化評価（v2.0、既存5次元スコアに追加）

⚠️ 念のため: 分析対象はヒューリスティック分析セクションに記述されたプロジェクト。
DDE ツール本体ではありません。

対象企業を **Atlas Associates のエンジニアリング哲学** で評価してください。
これは既存5次元スコアに **置き換わる** ものではなく、**並列に追加される** 評価軸です。
Atlas Associates は **DDE（この評価ツール）の開発元** であり、分析対象ではありません。
Atlas の哲学を **対象プロジェクトの評価** に適用してください（業界が何であれ）。

### 4軸（重み合計100%）

1. **高速化 Performance (20%)** — アルゴリズム効率 × ネットワーク最適化 × キャッシュ戦略 ×
   非同期・並列 × DB最適化 × 起動時間
...
4. **セキュリティ強度 Security Strength (55% — Atlas 哲学の核心)**

   サブ内訳（非公開重み、業界別動的、暗号化主軸）:

   - **4a. 暗号化技術の高度さ（35%、核心）** — Signal Protocol、PQXDH、libsignal/BoringSSL
...
### 重み哲学
Atlas は **サイバーセキュリティ対策全般**（暗号化 + プライバシー + 通信 + レイヤー防御）を
55% で重視する。中でも暗号化実装の深度（35%）が最大サブウェイト — **暗号化単独が
重要なのではなく、4 サブカテゴリ全体が真剣なサイバーセキュリティエンジニアリングを
示す**。第三者認証は **スコアに一切反映しない**:
- SOC2 取得済みでも平文保存のサービスは軸4で低評価（認証は関係ない）。
- SOC2 未取得でも libsignal + PQXDH + Sealed Sender + 多層防御のサービスは高評価。
...
本評価は **追加** — 既存の `dimension_scores`（5次元）は必ず埋めてください。
"""


_IMPLEMENTATION_MATRIX_INSTRUCTIONS_EN = """## Implementation Capability Matrix (v2.0, 8th competitive chart)

> ⚠️ **Reminder from STEP 0**: The competitor list and each cell's status must be
> backed by web-searched evidence (2025-2026). For every `verified` and `claimed`
> cell, the `sources` array must contain the specific URL where the evidence was
> found. Do not assert verification based on training-data memory alone.
...
> ⚠️ **STEP 0 リマインダー**: 競合リストと各セルの状態は Web 検索による
> 裏付け（2025〜2026 年情報）が必要です。`verified` / `claimed` の全セルに
> ついて、`sources` 配列に **証拠が見つかった具体的な URL** を記載すること。
> 学習データの記憶のみで verified を主張しないこと。
...
```json
{
  ...
  "data_freshness": {
    "web_search_performed": true,
    "search_date": "YYYY-MM-DD (the date you ran the searches, e.g. 2026-05-13)",
    "queries_executed": [
      "list every distinct search query you actually ran",
      "..."
    ],
    "sources_consulted": [
      "https://... (each URL you opened/fetched to verify a fact)",
      "..."
    ],
    "data_cutoff_warning": "null if web search succeeded; otherwise a clear warning string like 'WebSearch unavailable in this runtime; analysis based on training-data baseline (cutoff ~2025).'"
  },
        ...
        "charts": [
          {
            ...
            "data_points": [
              { "name": "Company A", "x": 0-100, "y": 0-100, "z": 0-100, "is_target": false, "last_verified": "YYYY-MM-DD", "sources": ["https://..."] },
              { "name": "Target Co", "x": 0-100, "y": 0-100, "z": 0-100, "is_target": true,  "last_verified": "YYYY-MM-DD", "sources": ["https://..."] }
            ]
          }
        ]
      }
  ...
  "atlas_four_axis": {
    "axes": [
      {"axis_key": "performance",  "name_en": "Performance",          "name_ja": "高速化",                "weight_pct": 20, "score": 0-100, "level": 1-10, "rationale": "...", "sub_items": []},
      ...
      {"axis_key": "security",     "name_en": "Security Strength",  "name_ja": "セキュリティ強度", "weight_pct": 55, "score": 0-100, "level": 1-10, "rationale": "...",
        "sub_items": [
          {"key": "encryption", "name_en": "Cryptographic Sophistication", "name_ja": "暗号化技術の高度さ", "weight_pct": 35, "score": 0-100, "level": 1-10, "rationale": "..."},
          {"key": "privacy",    "name_en": "Privacy Protection",            "name_ja": "プライバシー保護",     "weight_pct":  8, "score": 0-100, "level": 1-10, "rationale": "..."},
          {"key": "posture",    "name_en": "Basic Hygiene (MFA/WebAuthn)",  "name_ja": "基本衛生（MFA・WebAuthn）", "weight_pct":  2, "score": 0-100, "level": 1-10, "rationale": "..."},
    ...
    "items": [
      {
        ...
        "item_ja": "PQXDH / ML-KEM-1024",
        "statuses": [
          {"company_name": "Target Co",   "status": "verified|claimed|not_implemented|unknown", "evidence": "URL or citation", "sources": ["https://... (required for verified/claimed; can be empty for unknown/not_implemented)"], "last_verified": "YYYY-MM-DD"},
          {"company_name": "CompetitorA", "status": "verified|claimed|not_implemented|unknown", "evidence": "...",            "sources": ["https://..."], "last_verified": "YYYY-MM-DD"}
        ]
      }
    ]
  },
...
```bash
dde report --consulting "{temp_path}" --pdf --lang {lang}
```
