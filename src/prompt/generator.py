...
> This prompt was generated by **DDE (Due Diligence Engine)** with `--pdf` mode.
> The AI reading this should: evaluate the project → output JSON → generate PDF.
>
> **Do NOT print the JSON to the user.** Save it to a file and run the PDF command at the end.
"""

_CONSULTING_HEADER_JA = """# DDE コンサルティンググレード テクニカルデューデリジェンス

> このプロンプトは **DDE (Due Diligence Engine)** の `--pdf` モードで生成されました。
> このプロンプトを読んだAIは: プロジェクトを評価 → JSON出力 → PDF生成 を実行してください。
>
> **JSONをユーザーに表示しないでください。** ファイルに保存し、末尾のPDFコマンドを実行してください。
"""

_CONSULTING_EVAL_EN = """## Evaluation Task

You are a world-class technology consultant and senior software engineer.
Perform a comprehensive technical due diligence based on the heuristic data above.

**Read all code in the repository**, then produce:

1. **6-Dimension Scoring** (1-10 scale per dimension, with rationale and what it enables)
2. **SWOT Analysis** — concrete, evidence-based (not generic platitudes)
3. **Future Outlook** — product vision, viability, 1/3/5-year projections with confidence level
4. **Strategic Advice** — immediate actions, medium-term priorities, long-term vision
5. **Investment Thesis** — clear recommendation with risks, upside, and comparable companies
6. **Red Flag Re-evaluation** — correct heuristic false positives (AI-assisted rush commits are normal, etc.)
7. **Glossary Additions** — any additional technical terms that need explanation

**Guidelines:**
- Be specific and evidence-based. Cite file paths and code patterns.
- Do not pad with filler text. Every sentence should carry information.
- Non-engineers will read the PDF. Use clear, direct language.
- State your AI model name in the output (for attribution).
"""

_CONSULTING_EVAL_JA = """## 評価タスク

あなたは世界トップクラスのテクノロジーコンサルタント兼シニアソフトウェアエンジニアです。
上記のヒューリスティックデータに基づき、包括的なテクニカルデューデリジェンスを実施してください。

**リポジトリ内の全コードを読み取り**、以下を生成してください:

1. **6次元スコアリング**（各次元1-10、根拠と「何が可能になるか」を添えて）
2. **SWOT分析** — 具体的・エビデンスベース（一般論ではなく）
3. **将来性評価** — プロダクトビジョン、実現可能性、1/3/5年予測（信頼度付き）
4. **戦略アドバイス** — 即座のアクション、中期優先事項、長期ビジョン
5. **投資判断** — 明確な推奨度（根拠、リスク、アップサイド、類似企業）
6. **レッドフラグ再評価** — ヒューリスティックの誤検知を修正（AI開発のラッシュコミットは正常等）
7. **追加用語集** — 説明が必要な追加技術用語

**ガイドライン:**
- 具体的かつエビデンスベースで。ファイルパスやコードパターンを引用。
- 冗長な文を書かない。全ての文に情報を持たせる。
- 非エンジニアもPDFを読む。明確で直接的な言葉を使う。
- 使用AIモデル名を出力に含める（帰属のため）。
"""
...
def _consulting_pdf_command(result: AnalysisResult, lang: str) -> str:
    ...
    if lang == "ja":
        return f"""## 最終ステップ（必ず実行してください）

上記のJSON評価結果をファイルに保存し、PDFレポートを生成してください。

```bash
cat > /tmp/{filename} << 'ENDJSON'
{{ここにあなたのJSON評価結果を貼り付け}}
ENDJSON
...
**注意:**
- JSONを直接ユーザーに表示しないでください
- `cat` コマンドでファイルに保存してから `dde report` を実行してください
- PDFが生成されたら、そのファイルパスをユーザーに伝えてください
"""
    else:
        return f"""## Final Step (MUST execute)

Save your JSON evaluation to a file and generate the PDF report.

```bash
cat > /tmp/{filename} << 'ENDJSON'
{{paste your JSON evaluation here}}
ENDJSON
...
**Important:**
- Do NOT print the JSON directly to the user
- Save to file with `cat`, then run `dde report`
- Once the PDF is generated, tell the user the file path
"""
