...
def _make_chart(chart_type: str, title: str, title_ja: str,
    ...
    return MarketChart(
        ...
        x_axis_rationale=f"Measures how well companies perform on {x_label.lower()} — a key differentiator in this market.",
        x_axis_rationale_ja=f"{x_label_ja}を横軸に採用。この市場における主要な差別化指標。",
        y_axis_rationale=f"Captures {y_label.lower()} which determines long-term competitive advantage.",
        y_axis_rationale_ja=f"{y_label_ja}を縦軸に採用。長期的な競争優位性を左右する指標。",
        data_points=[
...
def _make_competitive_analysis() -> CompetitiveAnalysis:
    ...
    chart_defs = [
        ...
        ("security_posture", "Security & Privacy", "セキュリティ＆プライバシー",
         "Security Depth", "セキュリティ実装深度",
         "Privacy Readiness", "プライバシー準備度"),
        ("data_governance", "Data Governance", "データガバナンス",
         "Data Protection", "データ保護能力",
         "Transparency", "透明性＆説明責任"),
